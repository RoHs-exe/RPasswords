...
$("#setMoon,#setSun").click(async (e) => {
	const button = $(e.target);
	const currTime = localStorage.getItem('currTime');
	const setTime = currTime == 'day' ? 'night' : 'day';

	localStorage.setItem('currTime', setTime);
	localStorage.setItem('everyTime', setTime);

	alert("Тема изменена и она будет всегда теперь отоюражаться! Обновите страницу F5");
})
